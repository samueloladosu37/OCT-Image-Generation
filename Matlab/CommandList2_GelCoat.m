...
Data_H = table2array(RowDATAH);
Data_V = table2array(RowDATAV);
...
title('GelCoat-Row A-line (first 3800 samples)');
exportgraphics(gcf, 'figures/GelCoat-RowAline.png', 'Resolution', 300);
...
%% Figure 2: Row B-scan (3D — rotating GIF)
figure;
mesh(B_sc_data);
title('GelCoat-Row B-scan (unscaled)');
view(3);
exportgraphics(gcf, 'figures/GelCoat-RowBscan.png', 'Resolution', 300);   % static fallback
make_rotating_gif('figures/GelCoat-RowBscan_rotation.gif');
...
figure;
mesh(B_sc_ready);
title('GelCoat-Rescaled B-scan');
view(3);
exportgraphics(gcf, 'figures/GelCoat-RescaledBscan.png', 'Resolution', 300);   % static fallback
make_rotating_gif('figures/GelCoat-RescaledBscan_rotation.gif');
...
mesh(B_scan);
view(2);
title('OCT GelCoat-B-scan');
exportgraphics(gcf, 'figures/GelCoat-OCT_Bscan.png', 'Resolution', 300);